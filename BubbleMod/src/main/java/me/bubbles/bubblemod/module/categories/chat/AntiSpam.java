...
public class AntiSpam extends Mod {
    ...
    public AntiSpam() {
        ...
        antiSpamList=clientInstance.getAntiSpamList();
    }
    ...
    public void onChat(Text message, MessageSignatureData signature, MessageIndicator indicator, CallbackInfo ci) {
        ...
        try {
            blacklist = antiSpamList.getStringData();
    ...
    public void onTick() {
        if(ticks<tickUpdate.getValueInt()) {
            ...
            mc.inGameHud.getChatHud().addMessage(Text.of("§8[§b§lBubbleMod§8] §fBlocked messages: §c"+blocked));
            blocked=0;
            ticks=0;
        }
    }
